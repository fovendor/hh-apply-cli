...
- **Два режима поиска:**
    - **Автоматический (рекомендуемый):** Ищет вакансии, наиболее подходящие (С ТОЧКИ ЗРЕНИЯ HH.RU) под ваше резюме, с возможностью применить фильтры.
...
3.  **Настройка фильтров (для автоматического режима):** Скрипт последовательно спросит про желаемый формат работы, город и зарплату, чтобы уточнить результаты поиска.
...
- `PERIOD`: Период поиска в сутках.
- `SKIP_APPLIED`, `DEDUPE`: Включение/отключение защиты от повторных откликов (когда работодатели спамят одной и той же вакансией в разных городах).
