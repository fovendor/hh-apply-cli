...
- [Параметры работы программы](#параметры-работы-программы)
  - [Настройки поиска и откликов](#настройки-поиска-и-откликов)
...
- **Отчетность:** Каждый сеанс откликов сохраняется в CSV-отчёте. Отчёты складируются в каталоги по датам. В каждом отчёте содержится название резюме. Это удобно, когда вы откликаетесь разными резюме (с разных аккаунтов).
...
Настройка производится путём редактирования переменных внутри скрипта. Для изменения доступны три раздела:

### Настройки поиска и откликов

- `COVER_FILE`: Путь к вашему основному сопроводительному письму.
- `ROLE_IDS_CONFIG`: ID профессиональных ролей для поиска.
- `TEXT_INCLUDE` и `NEGATIVE`: Ключевые слова для фильтрации вакансий.
- `SKIP_APPLIED`: Установите `0`, чтобы по умолчанию НЕ пропускать отклики в компании, куда вы уже писали.
- `DEDUPE`: Установите `0`, чтобы по умолчанию НЕ удалять дубликаты вакансий.
...
По умолчанию цвета подобраны под тему оформления [Catppuccin for VSCode](https://github.com/catppuccin/vscode). Для каждого цвета есть комментарий.
...
2. Время, которое вы тратите на чтение описания, является естественной паузой между API-запросами, что делает поведение скрипта похожим на человеческое. **Важно!** Не рекомендуется быстро "пролистывать" весь список вакансий стрелками, это может быть расценено hh.ru как активность роботов и привести к временной блокировке сессии (потребуется капча, которую никак не ввести).

3. **Расположение кэша:** Все файлы кэша хранятся в директории `~/.cache/pm-search-cache/`. Если по какой-то причине вам необходимо очистить кэш, выполните команду:
    ```bash
    rm -rf ~/.cache/pm-search-cache
    ```
...
3. **Проверка на "сложные" вакансии:** Скрипт умеет определять вакансии, требующие ответа на вопросы во время отклика (`is_complex_vacancy`). Эта проверка надёжна, так как использует уже закэшированные данные. Отклики на такие вакансии будут пропущены, вы увидите это в консоли во время рассылки откликов. Также это найдёт отражение в CSV-отчёте, чтобы вы смогли откликнуться на такую вакансию вручную.
