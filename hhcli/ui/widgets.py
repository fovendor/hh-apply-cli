...
class Pagination(Horizontal):
    """
    Финальная, простая и надежная версия виджета пагинации.
    Стилизация полностью вынесена в CSS.
    """
    ...
    def _rebuild_controls(self) -> None:
        ...
        widgets = []
        # Кнопки "назад"
        widgets.append(Button("<<", id="first", disabled=self.current_page == 0))
        widgets.append(Button("<", id="prev", disabled=self.current_page == 0))
        ...
        # Кнопки "вперед"
        widgets.append(Button(">", id="next", disabled=self.current_page >= self.total_pages - 1))
        widgets.append(Button(">>", id="last", disabled=self.current_page >= self.total_pages - 1))
